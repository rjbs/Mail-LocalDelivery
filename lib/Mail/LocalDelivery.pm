...
# ----------------------------------------------------------
    ...
    foreach my $deliver_type (sort keys %deliver_types) {
        ...
        $self->_debug(3, "calling deliver handler $deliver_handler(@{$deliver_types{$deliver_type}})");
...
# ----------------------------------------------------------
    ...
    if (not $write_opts->{need_from} and $self->{email}->head->header->[0] =~ /^From\s/)
    {
    ...
    if ($write_opts->{need_from} and $self->{email}->head->header->[0] !~ /^From\s/) {
        $self->_debug(3,"No mbox From line, making one up.");
        if (exists $ENV{UFLINE}) {
            $self->_debug(3,"Looks qmail, but preline not run, prepending UFLINE, RPLINE, DTLINE");
            print FH $ENV{UFLINE};
            print FH $ENV{RPLINE};
            print FH $ENV{DTLINE};
        } else {
...
# ----------------------------------------------------------
    ...
    foreach my $file (my @maildirs = @_) {
        ...
        if (my $mkdir_error
          = $self->_mkdir_p($self->{one_for_all} ? ($file)
    ...
    foreach my $file (my @maildirs = @_) {
        ...
        # auto-create the maildir.
        if (my $mkdir_error = $self->_mkdir_p(
                                      $self->{one_for_all}
                                      ? ($file)
                                      : map { "$file/$_" } qw(tmp new cur))) { $self->_debug(0, $mkdir_error); next; }
        ...
        if    (link $tmp_path, $new_path) { push @saved_to, $new_path; }
            ...
            else { $self->_debug(0,"Couldn't link $tmp_path to $new_path: $!"); }
        }
    }
...
sub _audit_get_lock {
    ...
    for (1..10) {
        if (flock($FH, LOCK_EX)) { $self->_debug(4, "  successfully locked file $file"); return; }
